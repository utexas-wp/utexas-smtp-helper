...
1. Download the latest version of UTexas SMTP Helper WP at https://wcms.its.utexas.edu/download-utexas-smtp-helper-wp.php
...
Updates are provided by the WordPress Update API. Use either the WordPress UI to update this plugin, or use Terminus (`terminus wp <site>.dev -- plugin update utexas-smtp-helper-wp`).
...
### Release protocol
See `wp-content/plugins/utexas-smtp-helper-wp/.github/ISSUE_TEMPLATE/release.md`

### Download hosting
We maintain a convenience script at https://wcms.its.utexas.edu/download-utexas-smtp-helper-wp.php that provides a simpler way to download the plugin than going to GitHub to get the latest release. This script queries the GitHub repository for the latest release and redirects output to that URL:

```php
<?php

$url = 'https://api.github.com/repos/utexas-wp/utexas-smtp-helper-wp/releases/latest';
$headers = [
